class Card::BaseComponent < ViewComponent::Base
  def initialize(title:nil, category:nil, footer_text:nil, footer_icon:nil, color:nil, row_classes:nil, scroll:false, close_button:false, open_button:false)
    @title = title 
    @category = category
  ...
  def call
    content_tag(:div, class: @row_classes) do
      content_tag(:div, class: "card") do 
        (@title.present? ? render(Card::HeaderComponent.new(title: @title, subtitle: @category, color: @color, close_button: @close_button, open_button: @open_button)) : ''.html_safe) +
